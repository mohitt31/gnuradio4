...
// VAL_SEEDS from blocks/onnx/src/ex4_python/oc_snapshots.py:63 -- the fixed multi-peak
// validation-seed set regenerated identically every training-epoch snapshot throughout this
// project's ML training runs (that research tree is scheduled to be reorganised away, so the
// integer literal is pinned here rather than a path into it): VAL_SEEDS = (42, 71, 100, 183, 266,
// 200). At SyntheticPeakSpectrum's training-matching defaults (see SyntheticPeakSpectrum.hpp,
// whose defaults mirror ex1_training.py per qa_SyntheticPeakSpectrum.cpp's own test), seeds 42 and
// 266 design zero peaks, and seed 200's designed peaks are so broad and high-amplitude (up to 76,
// spanning most of the spectrum) that they contaminate PeakDetector's own residual noise estimate
// -- at PeakDetector's shipped defaults (see below) that scene reports zero detections, which makes
// it a poor worked example of ordinary detection. Seed 100 lands in a heavily overlapping region for
// half its designed peaks but still gives a clean sub-bin match on one narrow, isolated, high-SNR
// peak, making it the more legible demonstrator; the companion regression test
// (qa_PeakDetector.cpp) exercises this seed plus 42/266 (zero-peak) and 200 (contaminated-estimate
// limitation) in detail.
constexpr std::uint64_t kTrainingSeed = 100ULL;
...
int main() {
    ...
    std::println("training seed: {} (VAL_SEEDS, blocks/onnx/src/ex4_python/oc_snapshots.py:63), detector at default settings\n", kTrainingSeed);
