...
struct SyntheticPeakSpectrum : gr::Block<SyntheticPeakSpectrum<T>, gr::Resampling<1LU, 1LU>> {
    ...
    GR_MAKE_REFLECTABLE(SyntheticPeakSpectrum, in, out, spectrum_size, max_peaks, snr_min_db, snr_max_db, noise_level, peak_width_min_bins, peak_width_max_frac, baseline_slope_max, edge_margin, narrow_peak_prob, add_coloured_noise, add_edge_effects, add_spurious_spikes, seed);
...
private:
    ...
    [[nodiscard]] gr::DataSet<T> createSpectrum() {
        ...
        for (std::size_t p = 0; p < nPeaks; ++p) {
            ...
            const T    centre    = (margin >= N / 2) ? uniform(static_cast<T>(edgeMarginBins), static_cast<T>(N - edgeMarginBins)) : uniform(static_cast<T>(margin), static_cast<T>(N - margin));
            ...
            const T sigma = addPeak(spectrum, N, centre, widthBins, amplitude, shapeIdx);
    ...
    T addPeak(std::span<T> spectrum, std::size_t N, T centre, T width, T amplitude, PeakShape shape) {
        ...
        switch (shape) {
        case PeakShape::gaussian: return addGaussian(spectrum, N, centre, sigma, amplitude);
        case PeakShape::asymmetricGaussian: return addAsymmetricGaussian(spectrum, N, centre, sigma, amplitude);
        case PeakShape::lorentzian: return addLorentzian(spectrum, N, centre, sigma, amplitude);
        case PeakShape::sinc2: return addSinc2(spectrum, N, centre, sigma * T(2), amplitude);
        case PeakShape::parabolic: return addParabolic(spectrum, N, centre, sigma * T(1.5), amplitude);
        case PeakShape::pseudoVoigt: return addPseudoVoigt(spectrum, N, centre, sigma, amplitude);
        case PeakShape::dualGaussian: return addDualGaussian(spectrum, N, centre, sigma, amplitude);
        }
        std::unreachable();
    }
