...
// SyntheticPeakSpectrum's generateBaseline()/addNoise() sequence (baseline_slope_max=0.3,
// noise_level=1, add_coloured_noise/add_edge_effects/add_spurious_spikes=false) against a
// standalone gr::rng::Xoshiro256pp(42) + gr::rng::GaussianNoise<double>, per the documented
// algorithms in algorithm/include/gnuradio-4.0/algorithm/rng/{Xoshiro256pp,GaussianNoise}.hpp.
// They pin the block's bit-reproducibility contract for its peak-free (max_peaks=0) path.
namespace {
...
const boost::ut::suite<"SyntheticPeakSpectrum"> syntheticPeakSpectrumTests = [] {
    ...
    "default construction reports the ex1_training.py-matching defaults"_test = [] {
        ...
        expect(approx(block.peak_width_max_frac.value, 0.15f, 1e-6f));
        expect(approx(block.baseline_slope_max.value, 0.3f, 1e-6f));
        expect(approx(block.edge_margin.value, 0.05f, 1e-6f));
        expect(approx(block.narrow_peak_prob.value, 0.3f, 1e-6f));
